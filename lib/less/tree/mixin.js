...
tree.mixin = {};
tree.mixin.Call = function (elements, args, index, currentFileInfo, important,excluded) {
...
tree.mixin.Call.prototype = {
    ...
    eval: function (env) {
        var mixins, mixin, args, rules = [], match = false, i, m, f, isRecursive, isOneFound, rule, excluded = this.excluded,
            ...
            if ((mixins = env.frames[i].find(this.selector)).length > 0) {
                ...
                for (m = 0; m < mixins.length; m++) {
                    ...
                    if (mixin.matchArgs(args, env)) {  
                        ...
                        {
                            let oldRules = [...candidate.mixin.rules];
                            let rules = oldRules.filter(item => item.name !== undefined);
                            let ruleSets = oldRules.filter(item => item.name === undefined)
                            candidate.mixin = {...candidate.mixin, rules: []};
                            candidate.mixin.rules = rules.filter(item =>
                                !excluded.includes(item.name[0].value)
                            );
                            candidate.mixin.rules = [...candidate.mixin.rules, ...ruleSets];
                        }
