        ...
        parsers: parsers = {
            ...
            primary: function () {
                ...
                while (current)
                {
                    node = this.extendRule() || mixin.definition() || this.rule() || this.ruleset() ||
                        mixin.call(excludeParams) || this.comment() || this.rulesetCall() || this.directive();
                    if (node) {
                        root.push(node);
                    } else {
                        if (!($re(/^[\s\n]+/) || $re(/^;+/))) {
            ...
            mixin: {
                ...
                call: function (excl) {
                    ...
                    if(excl.length > 0)
                    this.exclude = [...this.exclude, ...excl];
                    ...
                    if (elements) {
                        ...
                        while(true){
                            if($char('-')){
                                let name = $getCurrent().replace(/;?[\n\r\s\t].*/g, '');
                                let rule = '';
                                if(name.includes('@@@')){
                                    error("variable expected", null, true);
                                }
                                if(!name.includes('@')){
                                    error("variable expected", null, true);
                                }
                                if(name.includes('@@')){
                                    let refTarget = input.slice(input.indexOf(name + ':')+1, input.indexOf(';',input.indexOf(name + ':'))+1);
                                    rule = input.slice(input.indexOf(refTarget), input.indexOf(';'));
                                }else{
                                    rule = input.slice(input.indexOf(name + ':')+((name+':').length), input.indexOf(';',input.indexOf(name + ':') + 1));
                                }
                                this.exclude = [...this.exclude, ...rule.slice(rule.indexOf(':')+1).replace(/[\"\s\']/g, '').split(',')];
                        ...
                        if(parsers.important()) {
                            restore();
                            error("the \"important\" modifier must precede any possible property removal operations.");
                        }
                        forget();
                        ...
                        if (parsers.end()) {
                            ...
                            const node = new(tree.mixin.Call)(elements, args, index, env.currentFileInfo, important, this.exclude);
                            this.exclude = [];
                            return node;
                        }
                    }
